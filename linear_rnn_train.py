...
from linear_rnn_build import Linear_Net
import torch
import torch.optim as optim
import torch.nn as nn
import os

# 建立线性近似网络, 此过程已经在显存中载入初始化网络了
...
# 选择初始化方法
optimizer = optim.SGD(linear_net.parameters(), lr=network_config.learning_rate)
...
write_result = open('./model/train_log/linear_net_training_dynamics.csv', 'w')

for epoch in range(network_config.total_epoch):
    optimizer.zero_grad()
    train_outputs = linear_net(train_x)
    ...
    # test_outputs = linear_net(test_x)
    ...
    if epoch % network_config.record_interval == 0:
        ...
        torch.save(linear_net, os.path.join(network_config.linear_rnn_dir, 'epoch_{:0>5d}_linear.pth'.format(epoch)))
